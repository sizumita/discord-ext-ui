...
class SampleView(View):
    ...
    async def show_alert(self, interaction: discord.Interaction):
        alert = Alert(
            "編集を終了しますか？",
            "",
            [ActionButton("いいえ", discord.ButtonStyle.blurple, False), ActionButton("はい", discord.ButtonStyle.danger, True)],
            ephemeral=True,
        )
    ...
    async def body(self):
        return (
            ...
            .items(
                [
                    Button("終わる").on_click(self.show_alert).style(discord.ButtonStyle.danger).disabled(self.content != "編集中..."),
                ]
            )
        )
